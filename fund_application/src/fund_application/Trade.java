...
//	}
	...
	public void Sell_stock(User_info User_id,Stock_info stock_name, int amount) {
		...
		if(having_stock>=amount) {
			...
			make_history(User_id, stock_name, amount, stock_price, time);
		}
...
//	}
	...
	private void make_history(User_info username,Stock_info stockname, String type, String time, String amount) {
		User_info.StockHis name = username.new StockHis();
		if(type=="buy"){
				
			name.buyhistory(Integer.parseInt(time), stockname.Stock_name, Integer.parseInt(amount));
		}
		else if(type=="sell") {
			name.sellhistory(Integer.parseInt(time), stockname.Stock_name, Integer.parseInt(amount));
		}
	}
}
